...
<head>
	...
	<link src="<?php echo base_url('asset/all.css') ?>" integrity="sha384-UHRtZLI+pbxtHCWp1t77Bi1L4ZtiqrqD80Kn4Z8NTSRyMA2Fd33n5dQ8lWUE00s/" crossorigin="anonymous"></script>   
				...
				<div class="panel-body">
					...
					<div class="col-md-13" align="right">
						<button  type="button"  id="add_button"  class="btn  btn-info  ">
							<i  class="material-icons" style="font-size:15px">add_circle</i> Add</button>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" language="javascript" >$(document).ready(function()
	...
	$(document).on('submit', '#user_form', function(event){event.preventDefault();$.ajax({url:"<?php echo base_url() . 'test_api/action' ?>",method:"POST",data:
		$(this).serialize(),dataType:"json",success:function(data){
			if(data.success){
				$('#user_form')[0].reset();
				$('#userModal').modal('hide');fetch_data();
	...
	document.getElementById('nim').readOnly = true;    
		var nim = $(this).attr('id');$.ajax({
		url:"<?php echo base_url(); ?>test_api/action",method:"POST",data:{nim:nim, data_action:'fetch_single'},dataType:"json",success:function(data){
			$('#userModal').modal('show');
			$('#nim').val(data.nim);                
			$('#nama').val(data.nama);
	...
	$(document).on('click', '.delete', function(){
		...
		if(confirm("Are you sure you want to delete this?")){
			$.ajax({url:"<?php echo base_url(); ?>test_api/action",method:"POST",data:{nim:nim, data_action:'Delete'},dataType:"JSON",success:function(data){
				if(data.success){
					$('#success_message').html('<div class="alert alert-success">Data Deleted</div>');fetch_data();
				}
			}
		})
	}});});
